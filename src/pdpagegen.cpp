...
QString PdPageGen::writeFile(DTAB *pTab)    // Create the html index page from the data table and embedded resource files.
{
    ...
    toStrm << ui->txtTitle->text() << Qt::endl;             // HTML <title>
    appendFile(":/html/snip2", wFile, toStrm);
    toStrm << ui->txtTitle->text() << Qt::endl;             // HTML <h1>
    appendFile(":/html/snip3", wFile, toStrm);
    toStrm << ui->txtDesc->toPlainText() << Qt::endl;       // Description below header
    ...
    {                                                   // Output subject lines.
        ...
        {
            toStrm << "<li>" << modl->data(QModelIndex(modl->index(i,0)), Qt::EditRole).toString() << "</li>" << Qt::endl;
        }
        toStrm << "</ul>" << Qt::endl;
    }
    appendFile(":/html/snip4", wFile, toStrm);
    toStrm << tr("Picture Information") << Qt::endl;        // Table caption.
    ...
    for (int i = 0; i < pTab->length(); i++)
    {
        toStrm << "<tr><td><a href=\"" << pTab->at(i).sFile << "\" target=\"_Blank\">" << pTab->at(i).sFile
               ...
               << "</td><td>" << pTab->at(i).sText << "</td></tr>" << Qt::endl;
    }
    appendFile(":/html/snip678", wFile, toStrm);                // End of table, data source and setup for version.
    toStrm << tr("Generated by PicDok version %1").arg(QApplication::applicationVersion()) << Qt::endl;
...
{
    inDelete = true;        // To stop lstSelChange deleting the same row if empty.
